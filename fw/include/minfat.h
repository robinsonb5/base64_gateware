...
typedef struct
{
	...
	uint32_t cursor;          /* Offset in bytes from the start of the file */
	uint32_t dirsector;       /* The sector containing the file's directory entry */
	uint32_t dirindex;        /* The entry number within the directory sector */
	int dirty;                /* The directory entry needs to be updated */
	int eof;
...
DIRENTRY *NextDirEntry(int prev,int (*matchfunc)(const unsigned char *fn,int len,void *userdata),void *userdata);
DIRENTRY *FindFreeDirEntry(fileTYPE *file);
DIRENTRY *ReadDirEntry(fileTYPE *file);
